<?php
/**
 * Render callback for the field-display block.
 *
 * @package {{namespace}}
 */
...
if ( ! function_exists( '{{namespace}}_render_field_display' ) ) {
	/**
	 ...
	 */
	function {{namespace}}_render_field_display( $attributes, $content, $block ) {
		...
		$wrapper_classes = array( 'wp-block-{{slug}}-field-display', 'wp-block-group', 'is-layout-flex', 'is-nowrap' );
		...
		if ( ! empty( $icon_name ) && function_exists( '{{namespace|snakeCase}}_get_icon_svg' ) ) {
