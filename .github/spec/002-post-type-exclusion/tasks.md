...
- [ ] T002 Add `content_model` property (`"type": "string"`, `"enum": ["none", "custom"]`, `"default": "custom"`) and the conditional constraint disallowing non-empty `post_types`/`taxonomies` when `content_model` is `"none"` to `.github/schemas/plugin-config.schema.json`, per [contracts/plugin-config-schema-delta.md](./contracts/plugin-config-schema-delta.md)
- [ ] T003 [P] In `scripts/generate-plugin.js`, derive `fullConfig.isFunctionalOnly = (config.content_model === 'none')` inside `applyDefaults()` (~line 283), alongside the existing `post_types`/`taxonomies` normalization, per research.md's "Where the generator reads the flag" decision
- [ ] T004 [P] In `scripts/generate-plugin.js`, add an explicit conflict check inside `validateConfig()` (~line 241): if `config.content_model === 'none'` and (`post_types.length > 0` or `taxonomies.length > 0`), return `{ valid: false, errors: [...] }` with a human-readable message naming both fields (FR-009), evaluated before/alongside the Ajv schema check
- [ ] T005 [P] In `scripts/validation/validate-plugin-config.js`, add a `validateContentModel(config)` helper mirroring the existing `validateTaxonomies(config)` pattern (~line 83), returning the same conflict error as T004 in array form, and export it from `module.exports`
